...
import flask
from ..common import ApiWrapper, security as security, api_wrapper2
...
def apiLogin():
    ...
    # call the api
    result = api_wrapper2.login(email, password)
...
def apiCreateAccount():
    ...
    api_response = api_wrapper2.createAccount(email=user_email, password=user_password, name_first=user_name_first, name_last=user_name_last, birth_date=user_birth_date)
...
def getProductImages(product_id: int):
    api = api_wrapper2.ApiWrapperProductImages(flask.g)
...
def getLocation(location_id: int):
    api_wrapper = api_wrapper2.ApiWrapperLocations(flask.g)
...
def getProductListingAvailability(product_id: int):    
    api = api_wrapper2.ApiWrapperListingAvailability(flask.g)
